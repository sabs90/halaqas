...
interface ShareButtonProps {
  url: string;
  text: string;
  trackingContext?: { event_id?: string; mosque_id?: string };
}

export function ShareButton({ url, text, trackingContext }: ShareButtonProps) {
  ...
  async function handleCopy() {
    try {
      ...
      setCopied(true);
      trackEvent('share_copy_url', trackingContext);
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 z-50 bg-white border border-sand-dark rounded-card shadow-lg py-1 min-w-[200px]">
          <a
            ...
            onClick={() => {
              trackEvent('share_whatsapp', trackingContext);
              setOpen(false);
            }}
          ...
          <a
            ...
            rel="noopener noreferrer"
            onClick={() => {
              trackEvent('share_facebook', trackingContext);
              setOpen(false);
            }}
            className="w-full px-4 py-2.5 text-left text-sm text-charcoal hover:bg-sand flex items-center gap-3 transition-colors"
          >
